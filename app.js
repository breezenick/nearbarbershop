...
app.get('/barbershops/:id/photos', async (req, res) => {
  const barbershopId = parseInt(req.params.id, 10);  // Parse ID to integer
  try {
    const barbershop = await Barbershop.findOne({ id: barbershopId });
    if (!barbershop || !barbershop.photos) {
      return res.status(404).json({ message: 'No photos found for this barbershop.' });
    }
    res.json(barbershop.photos);  // Return the photos as JSON
  } catch (error) {
